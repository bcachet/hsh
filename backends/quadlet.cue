...
#KubernetesPod: {
	...
	_traefikLabels: {
		...
		for portNum, port in _workload.expose.ports {
			if port.domain != _|_ {
				"traefik.http.routers.\(strings.Replace(port.domain, ".", "-", -1)).rule": "Host(`\(port.domain)`)"
				"traefik.http.routers.\(strings.Replace(port.domain, ".", "-", -1)).entrypoints": "https"
				"traefik.http.routers.\(strings.Replace(port.domain, ".", "-", -1)).tls": "true"
				"traefik.http.routers.\(strings.Replace(port.domain, ".", "-", -1)).tls.certresolver": "letsencrypt"
				"traefik.http.services.\(strings.Replace(port.domain, ".", "-", -1)).loadbalancer.server.port": "\(port.containerPort)"	
			}
		}
	}
	...
	_volumeMounts: list.Concat([
		...
		// Config volumes
		[for configName, config in _workload.configs {
			{
		...
		[for configName, config in _workload.configs {
			{
				name: "config-\(configName)"
				hostPath: {
					path: ".generated/\(_name)-\(configName)"
				}
			}
		}]
	])
